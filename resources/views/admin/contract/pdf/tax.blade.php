...
<p class="bold fs-9 text-center">ĐỊA CHỈ: {{ $data->transfer_land_title }} 
    {{ $data->transfer_land_address }}</p><br>
<p>Hôm nay ngày
...
<div>
    <ul>
        ...
        <li>Địa chỉ thường trú: {{ $data->transfer_customer_address }}</li>
    </ul>
</div><br>
...
<div>
    <ul>
        ...
        <li>ADD/Địa chỉ: 1016/19 (SC19-2) Sky Garden 1, Nguyễn Văn Linh, Phường Tân Phong, Quận 7, HCM</li>
...
<ul>
    ...
    <li>Căn cứ vào giấy chứng nhận chủ quyền số: <span
        ...
        <span class="bold">{{ date('d/m/Y', strtotime($data->transfer_land_certification_date)) }}</span> do ủy ban nhân dân quận 7 cấp
    </li>
</ul>
...
<div>
    <ul>
        <li>Tổng giá trị chuyển nhượng tài sản là: <span
                class="bold">{{ number_format($data->transfer_price_number) }} VND
             (Bằng chữ:{{ $data->transfer_price_text }})</span>
             <p><span class="bold"><i>Giá chuyển nhượng này bao gồm: phí chuyển nhượng tại phòng công chứng, 
                thuế thu nhập cá nhân theo qui định của chi cục thuế quận 7, phí môi giới.</i></span></p>
    ...
    <ul>
        <li><span class="bold">Đợt 1 : </span>Ngày {{ date('d', strtotime($data->transfer_payment1_date)) }} tháng
            ...
            {{ date('Y', strtotime($data->transfer_payment1_date)) }}. Bên B đặt cọc cho Bên A số tiền là : <span
                ...
                {{ $data->transfer_payment1_price_text }})</span> để mua căn hộ: <span class="bold">{{ $data->transfer_buy_apartment }}</span>
            </li>
        ...
        <li><span class="bold">Đợt 3 : </span> Ngay sau khi Bên A hoàn tất nghĩa vụ thuế TNCN, 
            Bên B thanh toan số tiền còn lại là: <span
            class="bold">{{ number_format($data->transfer_payment3_price_number) }}VND
            (Bằng Chữ: {{ $data->transfer_payment3_price_text}})</span>
        </li>
        <li><span class="bold"><i>Trong trường hợp Bên A chậm trế hoãn không hoàn thành nghĩa vụ thuế TNCN cho nhà nước, 
            Bên B sẽ đóng thay và trừ vào số tiền còn lại chưa thanh toán cho Bên A.</i></span></li>
    </ul>
</div><br>
<p class="mt"><span class="bold under">ĐIỀU 3: </span><span class="bold">TRÁCH NHIỆM VÀ QUYỀN HẠN CỦA BÊN A</span></p>
<div>
    <ul>
        ...
        <li>Đóng Thuế TNCN, phí công chứng, phí môi giới  cho Bên C.</li>
    </ul>
</div><br>
...
<p class="mt"><span class="bold under">ĐIỀU 5: </span><span class="bold">NGHĨA VỤ CỦA BÊN C</span></p>
<div>
    <ul>
...
<p class="mt"><span class="bold under">ĐIỀU 6: </span><span class="bold">CÁC ĐIỀU KHOẢN CHUNG</span></p>
...
<div>
    <ul>
        ...
        <li>Các vướng mắc, tranh chấp, khiếu nại  liên quan đến hợp đồng đặt cọc này  sẽ được các bên giải quyết trước 
            hết bằng thương lượng trên tinh thần hữu nghị và thiện chí . Sau mọi hòa giải nếu một bên nhận thấy tranh 
            chấp không thể giải quyết  thông qua hòa giải, thì bên này phải gửi một thông báo tranh chấp cho bên kia, 
            nếu sau 7 ngày kể từ ngày gửi thông báo mà tranh chấp vẫn không thê giải quyết bằng hòa giải thì một trong 
            hai bên có quyền đề nghị Tòa Án có thẩm quyền tại TPHCM giải quyết.</li>
    ...
    <p><i>  Hợp đồng này lập thành 3 bản chính. Mỗi bên giữ 01 bản có giá trị như nhau  và có hiệu lực kể từ ngày ký.</i></p>
