            ...
            <x-card class="card-default color-palette-box">
                <x-slot name="header">
                    <x-card.header class="d-flex justify-content-between">
                        <h3 class="card-title">
                            {{ __('Danh sách chủ nhà') }}
                            @isset($admin)
                                    - 
                                <x-link :href="route('admin.admin.edit', $admin->id)" :title="$admin->fullname" />
                            @endisset
                        </h3>
                        <x-link :href="route('admin.houseowner.create')" :title="__('Thêm chủ nhà')" class="btn btn-sm btn-primary shadow-sm ml-auto">
                            <x-link.icon class="fa fa-user-plus" />
                        </x-link>
                    </x-card.header>
                </x-slot>
                <div class="table-responsive">
                    {{$dataTable->table(['class' => 'table table-bordered', 'style' => 'min-width: 1200px'], true)}}
                </div>
            </x-card>
        </div><!-- /.col -->
    </x-admin.partials.page-content>
...
@include('admin.houseowner.script.datatable')
